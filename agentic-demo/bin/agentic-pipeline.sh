...
# ── 3. API 통합 → UI 빌드 (순차) ─────────────────────────────────
# api-integrator 가 먼저: types.ts + src/api/*.ts 를 만들어두면
# ui-builder 가 그것을 실제로 import 하면서 컴포넌트 + useEffect 연결까지 완성.
# (병렬로 돌리면 ui-builder가 존재하지 않는 import 를 추측해서 깨짐.)
run_agent api-integrator    "TASKS.md 기준으로 백엔드를 curl로 학습한 뒤 web-dashboard/src/api/, src/types.ts 작성. 컴포넌트는 손대지 말 것."
run_agent ui-builder        "TASKS.md 기준으로 web-dashboard/src/components/ 컴포넌트 작성. api-integrator가 만든 src/api/*.ts, src/types.ts 를 import 해서 useEffect 연결까지 본인이 마무리."
